...
else
{
    ...
    $sql_stat="SELECT DISTINCT * from ($day_part) as d,med_spec as ms ,medical as m, specialty as s,
    ...
    if(!$results)
        ...
        while ($med=$results->fetch_assoc()) 
        {
            ...
            $html=$html."<td ><b>سعر الكشف: </b>".$med["price"];
            $html=$html."<br>".$med["side_spec"];
            ...
            <?php
            $html=$html."<td>".'<button class="btn btn-success" onclick="reserve_hospital('.$med["med_id"].','.$med["spec_id"].');">إحجز</button>'."</td>";
